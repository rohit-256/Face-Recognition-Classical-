...
def Gaussian_Bayes(X_train,X_test,M):
    ...
    for i in range(N_test):
        for j in range(M):
            disc[i,j] = -0.5*determinants[j] + -0.5*(X_test[:,i]-class_means[:,j]).T @ sigma_inv[j,:,:] @ (X_test[:,i]-class_means[:,j])
